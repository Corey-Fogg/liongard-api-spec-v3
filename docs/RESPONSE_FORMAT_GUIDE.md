...
item = response.json()["data"]  # Had to unwrap

# GET list
response = requests.get(url)
result = response.json()
items = result["data"]  # Had to unwrap
pagination = result["pagination"]  # In body
...
# GET single
response = requests.get(url)
item = response.json()  # Direct access

# GET list
response = requests.get(url)
items = response.json()  # Direct array
has_more = response.headers.get('X-Pagination-Has-More') == 'true'  # In headers
```
